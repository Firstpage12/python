    ...
    with sqlite3.connect("student_detials.db") as connection:
        ...
        if not rows == []:
            try:
                ...
                with sqlite3.connect("student_detials.db") as connection:
                    cursor = connection.cursor()
                    cursor.execute("UPDATE Student_Info SET name=%s, email=%s, gender=%s, contact=%s, dob=%s, address=%s where id=%s",(name, email, gender, contact, dob, address))
                    connection.commit()
                    msg = "Student detials Updated"
            except:
                connection.rollback()
                msg = "We can not update Student detials to the database"
            finally:
                return render_template("success_record.html",msg = msg)
            connection.close()
